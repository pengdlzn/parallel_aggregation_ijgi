...
we must update the adjacent areas in order to maintain the topology.
If an adjacent area is involved in another merging event,
then it will be complicated to update the area for the two merging events.
Second, users can more easily understand the events 
than merging several areas into a single one,
...
First, the adjacent areas are blocked.
Second, the neighbors
of the adjacent areas are blocked (see \fig\ref{fig:blocked_polygons}a).
When searching for another merging event for the step, 
we select the least important area and its most compatible neighbor
from the free areas (see \fig\ref{fig:blocked_polygons}b);
...
Our case study will show that this way of compressing is very efficient
