...
(see \tbl\ref{tab:face_tgap_parallel}, the new values are underlined).
...
On the server side, we compute the values shown in \tbl\ref{tab:sequence_greedy}.
\tbl\ref{tab:sequence_greedy} is based on the merging steps of
...
(also see the $n_\mathrm{diff}$ value in the first row of \tbl\ref{tab:sequence_greedy}).
...
(also see the state\_end value in the first row of \tbl\ref{tab:sequence_greedy}).
...
%
\captionsetup*{type=table} %*: suppress warning "The caption type was already set to figure"
\caption{Some information of the merging sequence shown in \tbl\ref{tab:sequence_greedy}.
Column~$n_\mathrm{area}$ shows the area number of the map at starting state~$s$,
that is, $|M_s|$ of \eq\ref{eq:n_expect}.
}
\label{tab:sequence_greedy}
\centering
...
As a result, we have all the values of \tbl\ref{tab:sequence_greedy}.
...
starting at step~$i$.
Take the values of \tbl\ref{tab:sequence_greedy} for example, 
$j_0$ is~$3$ 
...
the $n_\mathrm{diff}$ values of \tbl\ref{tab:sequence_greedy}
...
(see column~$n_\mathrm{event}$ in \tbl\ref{tab:sequence_greedy}).
...
(see columns~state\_start and state\_end in \tbl\ref{tab:sequence_greedy}).
