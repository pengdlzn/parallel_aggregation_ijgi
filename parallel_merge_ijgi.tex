...
moved to adjust the boxes for the two maps.}
...
This expansion can be visualized based on 
slicing the space-scale cube (SSC) shown in
...
a set of events happening at the same interval of animation time.
For example, 
...
the animation time of zooming from one state to another state.
...
%This is different from earlier implementation, 
%where the compatibility of neighbours was defined 
%based on *both* the length of the common boundary and 
%the class of the neighbouring polygon. 
%Here, we could also involve the compactness of the result 
%into the equation for deciding 
%which neighbour is best (iff important that we want compact regions)
...
where parallel parameter~$r_\mathrm{parallel}$ is set to~$0.3$.
Note that this value is extremely high, 
just used to explain the principle in an artificial simple example.
...
for the smaller-scale map after a zooming-out operation.
...
is the number of area objects on the base map,
...
We will use the same interval of animation time and 
...
That is, some tiny and relatively unimportant areas stays 
...
happening in users' focused region for a time duration,
which may be too much workload for a user to follow;
for another time duration, 
