...
(\eg~\fig\ref{fig:intro}p is the same as \fig\ref{fig:intro}a).
...
which is a smooth merging going on.
...
\begin{figure}
\centering
\includegraphics[page=2,width=0.5\linewidth]{single_parallel_comparison}
\caption{The SSC of the merging strategy of \figs\ref{fig:intro}d--j.
...
\begin{subfigure}[b]{0.49\textwidth}
 \centering
 \includegraphics[page=2,width=0.95\linewidth]{single_parallel_comparison}
 \caption{$y=x$}
...
\begin{subfigure}[b]{0.49\textwidth}
 \centering
 \includegraphics[page=3,width=0.95\linewidth]{single_parallel_comparison}
 \caption{$y=3sinx$}
...
$j_1$ is~$3$ 
because the first three~$n_\mathrm{diff}$ values are the same (\ie~1),
and~$j_2$ is~$2$ 
because the following two~$n_\mathrm{diff}$ values are the same (\ie~0). 
...
\textbf{We consider the importance of an area as its size. 
That is to say the least important area is the smallest area.}
