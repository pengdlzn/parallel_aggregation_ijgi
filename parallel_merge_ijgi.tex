...
the animation duration can be short.
...
the animation duration of each event now is 
...
%at the same interval of animation time.
...
\url{https://pengdlzn.github.io/webmaps/2020/05/merge/example-discrete/}.}
...
For example, \fig\ref{fig:ssc_smooth_only}a is the SSC 
...
Please try the map at
\url{https://pengdlzn.github.io/webmaps/2020/05/merge/example-single-event/}.}
...
which is related to transition duration.
%
...
The dashed polygon shows the place where a user may focus;
in the place, the user has about twice of the duration 
to perceive the merging of the pink area, i.e., figures (k--l),
comparing to figures (f--g).
The numbers are ids of the faces, which will be used in \sect\ref{sec:methodology}.
}
\label{fig:intro}
\end{figure}
...
Please try the map at
\url{https://pengdlzn.github.io/webmaps/2020/05/merge/example-parallel-event/}.}
...
A comparison of the single-event merging (left) 
and the parallel-event merging (right) is at
\url{https://pengdlzn.github.io/webmaps/2020/05/merge/example-paralle-event/comparer.html}, 
...
The importance of an area is the multiplication 
of its size and its classification weight.
The compatibility value between a pair of areas is 
the multiplication of the length of the common boundary and 
...
they may put the face back with a higher importance, collapse it, 
...
The SSCs of \fig\ref{fig:ssc_smooth_only} was built 
...
We define the importance and the compatibility 
the same as \citet{vanOosterom2005}; see \sect\ref{sec:realted_work}.
Currently, we have not investigated on 
how to define a weight for a classification,
so we assign value~$1$ to the weights of all the classes.
As a result, the least important area is the one with smallest size.
We defined the compatibility of two classes 
based on the classification codes 
because the codes indeed imply a hierarchy.
\appx\ref{appx:create_tables} shows our implementation of
computing the weight values and the classification compatibilities.
By the implementation, the values are 
stored in tables of PostgreSQL database.
...
when the slicing arrives at the state\_low,
and the face should disappear
when the slicing arrives at the state\_high.
...
\caption{Some columns of the face tables. 
    Columns state\_low, state\_merge, and state\_low show the states 
    ...
    Column state\_merge is not really stored in the database.
    We show the column so that it is easy to see the differences 
    between the state\_low values and the state\_merge values.
    }
\label{tbl:face_tgap}
...
\begin{tabular}{cccc}
\toprule
face\_id& state\_low    & state\_merge & state\_high \\ \midrule
1       &     0         &     0         &     1       \\
2       &     0         &     0         &     1       \\
3       &     0         &     1         &     2       \\ 
4       &     0         &     4         &     5       \\
...
and its state\_low value will be~$s_\mathrm{high}$.
...
A simple way is to add a column, say, \emph{state\_merge} 
...
Therefore, we compute state\_merge values 
...
in \fig\ref{fig:ssc_smooth_only}a), and 
...
In order to snap to valid states,
...
On the server side, 
we compute the values shown in \tbl\ref{tbl:sequence_greedy}.
These values are for merging sequence of \fig\ref{fig:intro}k--o 
(also see \tbl\ref{tbl:face_tgap}b),
...
our target it to parallel three events ($n_{\mathrm{target},1} = 3$).
...
Furthermore, the state\_low value of step~$i+1$ (\ie~$s_{\mathrm{low},i+1}$) 
is the same as the state\_high value of step~$i$ (\ie~$s_{\mathrm{high},i}$).
...
and the state\_high value (\ie~$s_{\mathrm{high},i+1}$) 
...
Among them, we record the exceptions (\ie~the non-zero values) 
and the corresponding steps in a list.
The \emph{exception list} is~$[[1, 1]]$ for the example of \tbl\ref{tbl:sequence_greedy}.
For the pair of values in the inner square brackets,
the first one represents the step,
and the second value represents~$n_\mathrm{diff}$.
...
will be the same as the values on the server side
...
[0, 2, 4, 5, 6]$,
which is also the list of valid states
...
\begin{figure}[tb]
\centering
\includegraphics[page=1,draft=false]{data}
\caption{
...
This situation happens~$2{,}715$ times in total for all the steps
...
because of the previously found events.
This case happens~$1{,}382$ times in total for all the steps
when parallel parameter~$r_\mathrm{parallel}=0.01$
...
the exception list is~$[[1, 19], [2, 126], \dots, [77, 1]]$,
which has~$73$ pairs of values.
...
See the web map at
\url{https://pengdlzn.github.io/webmaps/2020/05/merge/single-event/}.} 
...
In order to show a better comparison of single-event merging 
and parallel-event merging,
...
the events of parallel-event merging 
can be better observed than the events of single-event merging.
This result shows the potential that
users can better keep their context 
