...
We define that an event is merging some parcels into a larger parcel. 
In our method, we require that each event involves exactly two parcels 
which is completely processed before the next pair is merged (all sequential). 
However, because of the given sequence in which the events are processed one by one, 
...
This presentation will show the details of finding and processing parallel events.
}
