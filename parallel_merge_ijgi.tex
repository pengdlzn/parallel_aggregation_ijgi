...
when the merging arrives at the state\_low value,
and the face should disappear
when the merging arrives at the state\_high value.
...
\frac{\sum\limits_{s \in S_\mathrm{high}} [s=s_\mathrm{end}]}{2},
$$
...
\tbl\ref{tab:sequence_greedy} shows an example of those values 
based on the merging steps of \fig\ref{fig:sequence_greedy}.
...
starting at state~$i$.
For example,the $n_\mathrm{diff}$ values of \tbl\ref{tab:sequence_greedy}
